...
class XMLTVGenerator:
    ...
    def generate(self, lineup_data, listings_by_day, source_url="http://localhost:8080"):
        ...
        lines.append(f'<tv date="{start_time}" source-info-url="{escape(source_url)}" source-info-name="tvtv2xmltv">')
    ...
    def _generate_channel(self, channel):
        ...
        return (f'<channel id="{channel_num}">'
    ...
    def _generate_programme(self, program, channel):
        ...
        lines = []
        lines.append(f'<programme start="{start_str}" stop="{end_str}" duration="{duration}" channel="{channel_num}">')
